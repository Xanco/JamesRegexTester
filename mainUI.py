...
class mainUI:
	...
	def TestTextAgainstRegex(self, Regex, TestText): # Define function TestTextAgainstRegex and get parameters Regex and TestText
		try:
	...
	#def PassLoadValueToRegex(Regex):
	...
	def SetupUI(self):
		...
		#RegexSaverVar = RegexSaver(root, self)
		...
		CheckText = Button(root, text="Test text against regex", command= lambda: self.TestTextAgainstRegex(regexEntry.get(), TestTextEntry.get())) # Create button with the text "Test text against regex" that onclick fired TestTextAgainstRegex and passes the value of regexEntry and TestTextEntry
		CheckText.pack()
